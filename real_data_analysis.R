...
############################################################
valid_genes <- rownames(RNA.count_BCells)  # same gene universe across all cell types
...
get_SNP_matrix <- function(gene_id, RNA_mat_adj, min_snps = 5) {
  ...
  snp.ind <- which(countOverlaps(vcf.ref, gene.promoter.ref[gene.ind]) > 0)
  ...
  t(genotype.mat[best_snps, , drop = FALSE])  # n x k matrix (donors x SNPs)
}
...
for (run in seq_len(nrow(lr_filtered))) {
  ...
  # Extract the keyword from the pathway name to find
  # corresponding genes in MSigDB (key_cols).
  ...
  # Six methods are computed and the one most correlated with X
  # is selected as the outcome variable. This avoids committing
  # to a single scoring method without biological justification.
  ...
  # ---- Step 4: Standardization scales (computed before centering) ----
  # SDs are unchanged by centering and are used to convert posterior
  # means back to interpretable standardized effect sizes.
  sd_X <- sd(as.numeric(X))
  sd_Z <- sd(as.numeric(Z))
  sd_Y <- sd(as.numeric(Y))

  # ---- Centering: enforce E[X] = E[Z] = E[Y] = 0 ----
  # Required by the paper's centering assumption (Proposition 1).
  ...
  # g-prior scales follow the paper's recommendation: min(n, 100).
  # Default hyperparameters: a_sigma=3, b_sigma=2, a_rho=3, b_rho=1, nu1=1e-4.
  n <- nrow(X_c)
  g_scale <- min(n, 100.0)
  ...
  # Convert from raw expression units to SD units of Y per SD of X (or Z).
  ...
  # Z_vec: receptor expression on the SD scale (for effect curves)
  # Effect_vec: total ligand effect as a function of receptor level
  #   effect(z) = beta_X^(s) + beta_XZ^(s) * z,  z = Z_c / sd_Z
  z_c    <- as.numeric(Z_c)
  Z_vec  <- z_c / sd_Z
...
# Bind all output rows into a single data frame
...
# ---- Helper: cell type display names (HTML-formatted) ----
fmt_cell <- function(x) {
  dplyr::case_when(
    x == "BCells"         ~ "B cells",
...
pair_label <- paste0(sender, " \u2192 ", receiver)  # e.g. "B cells -> Monocytes"
...
p2 <- ggplot(df_lollipop, aes(x = gamma_mean, y = LR_ranked)) +
  ...
  geom_segment(
    aes(x = 0, xend = gamma_mean, yend = LR_ranked, color = pathway_name),
...
# High-PIP curves are drawn prominently; low-PIP are faint.
# Top discovered triplets per pathway are labelled.
# ============================================================
df_lines <- Output_MR_CCC %>%
  ...
  select(triplet_id, pathway_name, LR, gamma_mean, pip_group, paired) %>%
  tidyr::unnest(paired) %>%
  group_by(triplet_id) %>%
  arrange(Z, .by_group = TRUE) %>%
  ungroup()

# Label the top 10 discovered triplets per pathway (by PIP)
df_labels <- df_lines %>%
  group_by(triplet_id, pathway_name, LR, gamma_mean) %>%
  slice_max(order_by = Z, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  group_by(pathway_name) %>%
  slice_max(order_by = gamma_mean, n = 10, with_ties = FALSE) %>%
  ungroup() %>%
  filter(gamma_mean >= pip_thresh)

p3 <- ggplot() +

  # Low-PIP curves: faint background
  geom_line(
    data      = df_lines %>% filter(pip_group == "low"),
    aes(x = Z, y = effect, group = triplet_id, color = gamma_mean),
    linewidth = 0.35, alpha = 0.15
  ) +

  # High-PIP curves: prominent foreground
  geom_line(
    data      = df_lines %>% filter(pip_group == "high"),
    aes(x = Z, y = effect, group = triplet_id, color = gamma_mean),
    linewidth = 1.15, alpha = 0.95
  ) +

  # Labels on high-PIP curve endpoints
  geom_text_repel(
    data               = df_labels,
  ...
  labs(
    ...
    subtitle = bquote(
      paste("Each curve: ",
            hat(beta)[X], " + ", hat(beta)[XZ],
            "  \u00b7  (Z / SD(Z));  ",
            "bold = PIP > ", .(pip_thresh),
            ";  faint = PIP \u2264 ", .(pip_thresh))
    ),
...
## running the script for different pairs produces separate files.
############################################################
